...
for i in raw_data['train']:
    ...
    count += 1
    if count % 2 == 0:
        train_texts.append(i['chosen']+ " [SEP] " + i['rejected'])
        train_sentiment.append(1)
    else:
        train_texts.append(i['rejected']+ " [SEP] " + i['chosen'])
        train_sentiment.append(0)
...
for i in raw_data['test']:
    ...
    count += 1
    if count % 2 == 0:
        test_texts.append(i['chosen']+ " [SEP] " + i['rejected'])
        test_sentiment.append(1)
    else:
        test_texts.append(i['rejected']+ " [SEP] " + i['chosen'])
        test_sentiment.append(0)
...
torch.save(train_dataset, "train_concat.pickle")
torch.save(test_dataset, "test_concat.pickle")